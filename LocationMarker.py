...
Location = collections.namedtuple(
    'Location', 'name description dimension position')

PREFIX = '!!loc'

STORAGE_FILE_PATH = os.path.join(
    'config', PLUGIN_METADATA['id'], 'locations.json')
            ...
            if not os.path.isfile(self.file_path):
                ...
                with open(self.file_path, 'r', encoding='utf8') as handle:
                    try:
                        ...
                        (logger.error if logger is not None else print)(
                            'Fail to load {}: {}'.format(self.file_path, e))
                        needs_overwrite = True
            if needs_overwrite:
                self.save()
...
def get_dim_key(dim: Union[int, str]) -> str:
    dimension_convert = {0: 'minecraft:overworld', -
                         1: 'minecraft:the_nether', 1: 'minecraft:the_end'}
...
def print_location(location, printer: Callable[[RTextBase], Any]):
    ...
    if config['teleport_hint_on_coordinate']:
        coordinate_text.c(RAction.suggest_command, '/execute in {} run tp {} {} {}'.format(
            get_dim_key(location.dimension), x, y, z)).h('点击以传送')
    printer(RTextList(
        '§7-§r ',
        name_text.h('点击以显示详情').c(RAction.run_command,
                                 '{} info {}'.format(PREFIX, location.name)),
        ' ',
        coordinate_text,
        ' §7@§r ',
...
def list_locations(source: CommandSource, *, keyword=None, page=None):
    ...
    if page is None:
        ...
        left, right = (page - 1) * \
            config['item_per_page'], page * config['item_per_page']
...
def show_location_detail(source: CommandSource, name):
    ...
    if loc is not None:
        ...
        source.reply(
            RTextList('坐标: ', get_coordinate_text(loc.position, precision=4)))
        source.reply(RTextList('详情: ', RText(
            loc.description if loc.description is not None else '无', color=RColor.gray)))
        x, y, z = map(round, loc.position)
        source.reply('VoxelMap路标: [name:{}, x:{}, y:{}, z:{}, dim:{}]'.format(
            loc.name, x, y, z, loc.dimension))
    else:
        source.reply('未找到路标§b{}§r'.format(name))
