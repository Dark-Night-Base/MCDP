...
def refresh_uuid_list(server: ServerInterface):
    ...
    if os.path.isfile(UUIDFilePrev):
        ...
        server.logger.info('Migrated {} uuid mapping from the previous {}'.format(
            len(uuid_file), os.path.basename(UUIDFilePrev)))
    # compatibility ends
        ...
        with codecs.open(file_name, 'r', encoding='utf8') as f:
            try:
                for item in json.load(f):
                    ...
                    expired_time = time.strptime(
                        item['expiresOn'].rsplit(' ', 1)[0], '%Y-%m-%d %X')
...
def save_uuid_list():
    global uuid_list
    uuid_list = dict(
        sorted(uuid_list.items(), key=lambda x: x[0].capitalize()))
...
def show_stat(server, info, name, cls, target, is_uuid, is_tell):
    ...
    msg = '玩家§b{}§r的统计信息[{}]的值为§a{}§r'.format(
        name, get_display_text(cls, target), get_stat_data(uuid, cls, target))
...
def show_rank(server, info, cls, target, list_bot, is_tell, is_all, is_called=False):
    ...
    if not is_called:
        print_message(
            server, info,
            '统计信息[{}]的总数为§c{}§r，前{}名为'.format(
                get_display_text(cls, target), sum, show_range),
            is_tell
        )
    ...
    for i in range(show_range):
        text = '#{}{}{}{}{}'.format(
            ...
            arr[i].name,
            ' ' * (1 if is_called else max_name_length -
                   len(arr[i].name) + 2),
            arr[i].value
        )
        ret.append(text)
        if not is_called:
            print_message(server, info, rankColor[min(
                i, len(rankColor) - 1)] + text, is_tell)
...
def build_scoreboard(server, info, cls, target, title=None, list_bot=False):
    ...
    server.execute('scoreboard objectives add {} minecraft.{}:minecraft.{} {}'.format(
        ScoreboardName, cls, target, title))
    for name, uuid in player_list:
        value = get_stat_data(uuid, cls, target)
        if value is not None:
            server.execute('scoreboard players set {} {} {}'.format(
                name, ScoreboardName, value))
...
def on_info(server, info: Info, arg=None):
    ...
    def inner():
        ...
        if cmdlen == 5 and command[1] == 'query':
            show_stat(server, info,
                      command[2], command[3], command[4], is_uuid, is_tell)
        ...
        # !!stats scoreboard [统计类别] [统计内容] [<标题>] (过滤bot前缀)
            ...
            build_scoreboard(
                server, info, command[2], command[3], title=title, list_bot=list_bot)
